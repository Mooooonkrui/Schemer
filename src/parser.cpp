...
Expr List::parse(Assoc &env) {
    if (stxs.empty()) throw RuntimeError("Syntax Invalid");
    if (auto first_one = dynamic_cast<Identifier *>(stxs.front().get())) {
        ...
        if (match_reserved_word != reserved_words.end()) {
            ...
            if (stxs.size() == 1) throw RuntimeError("Syntax Invalid");
        ...
        switch (flag) {
            case E_LET: {
                ...
                if (stxs.size() != 3) throw RuntimeError("Syntax Invalid");
            ...
            case E_IF: {
                if (stxs.size() != 4) throw RuntimeError("Syntax Invalid");
            ...
            case E_QUOTE: {
                if (stxs.size() != 2) throw RuntimeError("Syntax Invalid");
